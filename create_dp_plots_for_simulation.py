...
def plot_metrics(allele_fractions: np.ndarray, 
    ...
    plt.ylabel('Score')
    plt.title('PPA and PPV as a function of the Total depth')
    plt.legend()
    plt.savefig('Somatic mutation simulation - DP vs PPA and PPV.png', dpi=200)
...
def main():
    ...
    parser.add_argument('-D','--mean_total_depth',     type=float, default=300,   help='Mean of the Poisson distribution, from which depth values are drawn',nargs='+')
    ...
    parser.add_argument('-a','--af',                   type=float, default=[0.2], help='The intrinsic Allele Fraction of somatic variants. The number of ALT reads due to a somatic variant is drawn from BD(D,AF)')
    ...
    plot_metrics(depths, ppas, ppvs)
    print(list(zip(depths, ppas, ppvs)))
