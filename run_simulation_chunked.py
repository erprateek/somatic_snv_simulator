...
def simulate_somatic_mutations_chunk(chunk_size, depth, error_rate, allele_fraction, alt_threshold, fv):
    ...
    total_depth = np.random.poisson(depth, size=chunk_size).astype('uint8')

    # Simulate sequencing noise from a Binomial distribution
    noise_reads = np.random.binomial(total_depth, error_rate).astype('uint8')

    # Simulate somatic mutation (if any)
    has_mutation = np.random.choice([True, False], size=chunk_size, p=[fv, 1-fv])
    mutation_reads = np.random.binomial(total_depth, allele_fraction).astype('uint8') * has_mutation
...
parser = argparse.ArgumentParser(
    description='Monte-Carlo simulation of somatic mutations in NGS data')
parser.add_argument('--num_simulations', type=int, default=1000, 
                    help='Number of simulations')
parser.add_argument('--depth', type=int, default=50, 
                    help='Total read depth')
parser.add_argument('--error_rate', type=float, default=0.05, 
                    help='Sequencing error rate')
parser.add_argument('--alt_threshold', type=int, default=3, 
                    help='ALT threshold')
parser.add_argument('--allele_fractions', nargs='+', type=float, default=[0.1],
                    help='Allele fractions to test')
parser.add_argument('--fv', type=float, default=0.5, 
                    help='Fraction of k pileups with a somatic variant')
...
for allele_fraction in args.allele_fractions:
    ...
    if remaining_simulations > 0:
        pileups, labels = simulate_somatic_mutations_chunk(remaining_simulations, args.depth, args.error_rate, allele_fraction, args.alt_threshold, args.fv)
        predictions     = pileups >= args.alt_threshold
        true_positive, false_positive, true_negative, false_negative = calculate_metrics(labels, predictions)
    ...
    confusion_matrix = np.array([[true_negative_sum, false_positive_sum], [false_negative_sum, true_positive_sum]])
    ppa = np.array(true_positive_sum) / (np.array(true_positive_sum) + np.array(false_negative_sum))
    ppv = np.array(true_positive_sum) / (np.array(true_positive_sum) + np.array(false_positive_sum))
    specificity = np.array(true_negative_sum) / (np.array(true_negative_sum) + np.array(false_positive_sum))


    print("Confusion Matrix:")
    print(confusion_matrix)
    print("PPA:", round(ppa*100,2))
